...
echo.
call :ColorText 19 "Mumble v0.1.1"
echo.
call :ColorText 19 "---"
echo.
...
echo.
ren %EXE_PATH% "mumble.exe

IF EXIST %RESOURCER_PATH% (
	call :ColorText 19 "Resourcer.exe exists, setting Mumble icon"
	echo.
	IF EXIST %ICO_PATH% (
...
echo.
cd %BUILD_DIR%
%ZIP_EXE% a -tzip "mumble_win64.zip" %BUILD_DIR%\locales %BUILD_DIR%\lib %BUILD_DIR%\node_modules %BUILD_DIR%\jspm_packages %BUILD_DIR%\*.*
