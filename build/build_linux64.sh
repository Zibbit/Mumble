#!/bin/bash
# Mumble Linux shell script

echo "Start creating Mumble binary"
...
APPEXE_PATH=$CURR_DIR/linux64/mumble
ZIP_EXE="/usr/bin/7z"
...
if ! [ -e "$APPEXE_PATH" ]; 
then
echo "error: failed to create mumble executable"
exit 1
else 
echo "mumble executable is created"
fi

echo "create zip file"

cd $BUILD_DIR
$ZIP_EXE a -tzip "mumble_linux64.zip" $BUILD_DIR/locales $BUILD_DIR/lib $BUILD_DIR/node_modules $BUILD_DIR/jspm_packages $BUILD_DIR/*.* $APPEXE_PATH

md5sum $BUILD_DIR/mumble_linux64.zip > $BUILD_DIR/mumble_linux64.md5

echo "creating Mumble binaries completed"
