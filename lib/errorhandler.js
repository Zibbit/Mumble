...
(function () {
    ...
    function handleFindMessages(err) {
        if (!err) {
            return mumble.notify.error("Error in finding messages: unknow error", null, true);
        }

        if ((err.message && typeof err.message == "string" && err.message.indexOf(NOMSGERR) > -1) ||
            (typeof err == "string" && err.indexOf(NOMSGERR) > -1)) {
            mumble.notify.info("There are no messages for the account", null, true);
        }
        else {
            mumble.notify.error("Error in finding messages:  %j", err, null, true);
        } 
    }
    ...
    function handleWebrtcError(err, param) {
        ...
        mumble.notify.error(msg, null, true);
    }
