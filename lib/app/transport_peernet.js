...
Peer to peer implementation of the Mumble network
...
function TransportPeerNet() {
    ...
    this.create_netclient = function (completefn) {
        var self = this;
        try {
            logger.debug("Initializing peer Mumble client");
                ...
                function (seeds, callback) {
                    try {
                        if (!seeds || !Array.isArray(seeds) || !seeds.length) {
                            return callback("Error in populating the seed list. Please make sure the 'bootseeds' configuration is correct and a firewall doesn't block the Mumble software!");
                        }
    ...
    this.setUpnpPort = function (callback) {
        try {
            ...
            client.portMapping(
                ...
                function (err, result) {
                    if (err) {
                        mumble.notify.error("UPNP portMapping error: %j", err.message ? err.message : err, true);
                    }
            ...
            mumble.notify.error("UPNP portMapping exception: %j", e, true);
            callback(e);
        }
    }    
    ...
    this.onPeerMessage = function(message, info) {
        try {
            if (!message) {
                return mumble.notify.error("Invalid message at onPeerMessage", null, true);
            }
            if (!message.type || message.type != "PEERMSG") {
                return mumble.notify.error("Invalid message type at onPeerMessage", null, true);
            }
            if (!message.data) {
                return mumble.notify.error("Invalid message data at onPeerMessage", null, true);
            }
    ...
    this.initNetwork = function (callback) {
        try {
            ...
            var contact = kad.contacts.MumbleContact(param);
