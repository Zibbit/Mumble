...
Ssdp.prototype.search = function search(device, promise) {
    ...
    function ondevice(info, device_address, socket_addess) {
        if (promise._ended) return;
        if (info.st !== device) return;

        promise.emit('device', info, device_address, socket_addess);
    }
...
Ssdp.prototype.createSocket = function createSocket(interface) {
    ...
    var socket = dgram.createSocket(interface.family === 'IPv4' ? 'udp4' : 'udp6');
