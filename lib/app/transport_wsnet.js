...
Websocket implementation of the Mumble network
...
function TransportWsNet(address, port) {
    ...
    this.protocol = mumble.globals.wsprotocol || "https";
    this.wssocket = 0;
    this.host = 0;
    this.port = port;
    ...
    this.onPerrMsg = function(data) {
        try {
            ...
            if (!message) {
                return mumble.notify.error("Invalid message at web socket peermsg");
            }
            if (!message.type || message.type != "PEERMSG") {
                return mumble.notify.error("Invalid message type at web socket peermsg");
            }
            if (!message.data) {
                return mumble.notify.error("Invalid message data at web socket peermsg");
            }
    ...
    this.create = function (host, port, callback) {
        ...
        socket.on("connect_error", function (err) {
            mumble.notify.error("web socket connect error: %j", err, true);
        });

        socket.on("reconnect_failed", function (err) {
            mumble.notify.error("Web socket reconnect error", null, true);
        });

        socket.on('error', function (err) {
            mumble.notify.error("web socket error: %j", err, true);
        });
        ...
        socket.on('connect', function () {
            ...
            socket.emit('register_account', request, function (err) {
                ...
                mumble.notify.info("WS connected socketid: " + socket.id, null, true);
            });            
        });
        ...
        socket.on('reconnecting', function (number) {
            mumble.notify.info("WS reconnecting count: " + number, null, true);
        });  

        socket.on('reconnect', function (number) {
            mumble.notify.info("WS reconnected following " + number + " attempt", null, true);
        }); 
        ...
        try {
            ...
            this.get_socket(function (sockerr, socket) {
                ...
                socket.emit("peermsg", request, function (err) {
                    if (err) {
                        return mumble.notify.error("WS peer message send error: %j", err, true);
                    }
                });
