...
(function () {
    ...
    define(['knockout', 'settings', 'logger', 'definitions', './settings.html!text'], function (ko, settings, logger, defs, template) {
        ...
        function SettingsVm() {
            ...
            var viewModel = {
                ...
                nwmode: ko.observable(mumble.globals.nwmode),
                ...
                init: function (callback) {
                    try {
                        ...
                        settings.getdata(function (err, data) {
                            try {
                                if (err) {
                                    return mumble.notify.error("Settings init error: %j", err);
                                }
                ...
                save: function () {
                    try {
                        ...
                        settings.update(data, function (err) {
                            if (err) {
                                mumble.notify.error("Error in updating the settings database. Error: " + err.message);
                            }
                            else {
                                logger.debug("Updated settings in database");
                                mumble.notify.success("The settings data was updated successfully");
                            }
                        });
                    }
                    catch (e) {
                        return mumble.notify.error("Exception occured in updating the settings database. Error: " + e.message);
                    }
                },
                ...
                add_bootseed: function () {
                    ...
                    if (!seed_host || !seed_port || isNaN(parseInt(seed_port))) {
                        return mumble.notify.error("Invalid seed data. The host IP address or domain name and port key are required.");
                    }
