...
(function () {
    ...
    define(
        ['knockout', 'appsrvc', 'contactlist', 'peercomm', 'appevents', 'apputils', 'mumblenet', 'settings', 'contactsutil', 'uihandler', 'utilities', './contactsbar.html!text'],
        function (ko, appsrvc, contactlist, peercomm, appevents, apputils, mumblenet, settings, contactsutil, uihandler, utilities, template) {
            ...
            var viewModel = {
                ...
                onContactError: function (account, error) {
                    try {
                        ...
                        mumble.notify.error("contact onContactError error %j", err, true);
                    }
                },
                ...
                onLastPing: function (account) {
                    try {
                        ...
                        mumble.notify.error("contact onLastPing error %j", err, true);
                    }
                },
                ...
                onContactOnline: function (account, isonline) {
                    try {
                        ...
                        mumble.notify.error("contact onContactOnline error %j", err, true);
                    }
                },
                ...
                onTextMessage: function (data) {
                    try {
                        ...
                        mumble.notify.error("contact onTextMessage error %j", err, true);
                    }
                },
                ...
                onFileReceive: function (sender, blob, filename, filesize) {
                    try {
                        ...
                        mumble.notify.error("contact onFileReceive error %j", err, true);
                    }
                },
                ...
                itemAction: function (item) {
                    try {
                        if (item.actiontype == "textmsg") {
                            ...
                            item.actiontype = "";
                            item.actionicon("");
                            mumble.notify.error("Contact error: %j", item.error());
                            item.error("")
                        }
                    }
                    catch (err) {
                        mumble.notify.error("contact itemAction error %j", err, true);
                    }
                },
                ...
                search: function () {
                    try {
                        ...
                        if (!account) {
                            return mumble.notify.error("Enter the human or device account name");
                        }

                        if (contactlist.exists(account)) {
                            return mumble.notify.info("This contact exists in the contact list");;
                        }

                        mumblenet.find_contact(account, function (err, contact) {
                            if (err) {
                                return mumble.notify.error('Error in searching for contact "' + account + '", %j', err);
                            }

                            if (!contact || !contact.account) {
                                return mumble.notify.info("Couldn't find contact " + account, null, true);
                            }                            
                        ...
                        mumble.notify.error("contact search error %j", err, true)
                    }
                },
                ...
                onSendAddContactRequest: function () {
                    ...
                    try {
                        ...
                        if (this.addcontact_obj.account == appsrvc.username) {
                            return mumble.notify.error("Can't add yourself as a contact.");
                        }
                        ...
                        contactsutil.add_contact(contact, function (err) {
                            ...
                            if (err) {
                                return mumble.notify.error("Add contact error %j", err);
                            }
                        ...
                        uihandler.unblockwin();
                        unblocked = true;
                        mumble.notify.error("Add contact error %j", err);
                    }
                ...
                acceptAddContact: function (obj) {
                    try {
                        ...
                        mumble.notify.error("Accept contact error %j", err, true)
                    }
                },
                ...
                declineAddContact: function (obj) {
                    try {                        
                        ...
                        mumble.notify.error("Decline contact error %j", err);
                    }                    
                },
