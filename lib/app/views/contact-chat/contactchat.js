            ...
            var viewModel = {
                ...
                init: function () {
                    try {
                        ...
                        mumble.notify.error("Chat view error %j", err);
                    }
                },
                ...
                sendchat: function () {
                    try {
                        if (!this.issession()) {
                            return mumble.notify.error("Unable to send chat message. No session exists with the contact");
                        }
                        ...
                        mumble.notify.error("Send chat error %j", err);
                    }
                },

                sendfile: function () {
                    if (!peercomm.is_peer_session(viewModel.contact.name)) {
                        return mumble.notify.error("Invalid contact session");
                    }
                ...
                onConnectionClosed: function (contactname) {
                    if (contactname == viewModel.contact.name) {
                        ...
                        mumble.notify.error("The connection with contact " + contactname + " was closed", null, true);
                    }
                }
            };
