...
(function () {
    ...
    define([
        'knockout', 'appsrvc', 'appevents', 'bs58check', 'contactsutil', 'peermsg', 'uihandler', 'utilities', './acceptcontrequest.html!text'],
        function (ko, appsrvc, appevents, bs58check, contactsutil, peermsg, uihandler, utilities, template) {
            ...
            var vm = {
                ...
                parse: function (value) {
                    try {
                        ...
                        var lookupstr = "MUMBLE CONTACT OFFER ---";
                        var pos = value.indexOf(lookupstr);
                        if (!pos < 0) {
                            vm.showcontent(false);
                            return mumble.notify.error("Invalid contact request.");
                        }

                        var tmp1 = value.substring(pos + lookupstr.length);
                        pos = tmp1.indexOf("--- END MUMBLE CONTACT OFFER");
                        ...
                        if (!decodedmsg) {
                            return mumble.notify.error("Signature verification failed. Invalid contact offer object.");
                        }
                     
                        var obj = JSON.parse(decodedmsg.data);
                        if (!obj.account || !obj.public_key || !obj.usertype || !obj.port || !obj.address || !obj.transport ) {
                            vm.showcontent(false);
                            return mumble.notify.error("Invalid contact offer object.");
                        }

                        if (payload.iss != obj.public_key) {
                            vm.showcontent(false);
                            return mumble.notify.error("Invalid contact offer object. Public key mismatch.");
                        }
                        ...
                        vm.showcontent(false);
                        mumble.notify.error("Error in parsing the contact offer object.");
                    }
                }
            };
