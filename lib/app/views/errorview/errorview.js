...
(function () {
    define(['knockout', './errorview.html!text'], function (ko, template) {
        function ErrorWiewModel(params) {
            this.error = ko.observable(params && params.error ? params.error : "Unknow error");
        }

        return {
            viewModel: ErrorWiewModel,
            template: template
        };
    });
}());
