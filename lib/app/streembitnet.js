...
(function () {
    ...
    var PeerNet = require('./transport_peernet');
    ...
    function netfactory(transport) {
        if ((mumble && mumble.globals && mumble.globals.nwmode) && transport == defs.TRANSPORT_TCP) {
            if (!settings.tcpport) {
            ...
            return new WsNet(settings.wshost, settings.wsport);
        }
    }
    ...
    peernet.peer_send = function (contact, data) {
        function get_contact_transport(contact) {
            if (appsrvc.transport == defs.TRANSPORT_WS) {
                //  this user is using web socket, must return that and use WS
                //  in communicating with the contact
                if (!(peernet.netclient instanceof WsNet)) {
                    throw new Error("invalid WS net instance");
                }
            ...
            if (!(peernet.netclient instanceof PeerNet)) {
                throw new Error("invalid PeerNet instance");
            }             
    ...
    peernet.put = function (key, value, callback) {
        ...
        var put_via_ws = function (k, v, donecb) { 
            var protocol = mumble.globals.wsprotocol || "https";
            var url = protocol + '://' + settings.wshost + ":" + settings.wsport + "/put";
        ...
        var wspeer_proc = function (k, v) {
            if (!(peernet.netclient instanceof WsNet)) {
                return callback("invalid WS net instance");
            }
        ...
        var dhtpeer_proc = function (k, v) {
            ...
            if (!(peernet.netclient instanceof PeerNet)) {
                return callback("invalid PeerNet instance");
            }
            peernet.netclient.put(k, v, callback);
            ...
            if (self.validate_ws_publish()) {
                ...
                put_via_ws(k, v, function (err, response) {
                    if (err) {
                        mumble.notify.error("WS put error %j", err, true);
                    }
                    if (!response || !response.hasOwnProperty("result") || response.result != 0) {
                        mumble.notify.error("Invalid websrvc response or websrvc failed", null, true);
                    }
                    else {
        ...
        //
        //  the handler for each transport type
        if (appsrvc.transport == defs.TRANSPORT_WS) {
            wspeer_proc(key, value);
        }
        else {
    ...
    peernet.register_at_ws = function () {
        ...
        var resultfunc = function (err, data) {
            if (err) {
                mumble.notify.error("WS register error %j", err, true);
            }
            else if (!data || !data.hasOwnProperty("result") || data.result != 0) {
                mumble.notify.error("Invalid websrvc response at contact register", (data && data.error) ? data.error : null, true);
            }
            else {
        ...
        var protocol = mumble.globals.wsprotocol || "https";
        var url = protocol + '://' + settings.wshost + ":" + settings.wsport + "/register_contact";
        var data = {
    ...
    peernet.find_contact = function (account, callback) {
        ...
        if(appsrvc.transport == defs.TRANSPORT_WS) {
            ...
            var resultfunc = function (err, data) {
                if (err) {
                    ...
                    if (!contact.address) {
                        ...
                        contact.address = settings.wshost;
                    }
                    if (!contact.port) {
                        // use the same address, the contact data must be there
                        contact.port = settings.wsport;
                    }
            ...
            var protocol = mumble.globals.wsprotocol || "https";
            var url = protocol + '://' + settings.wshost + ":" + settings.wsport + "/find_contact";
            var data = {
                "account": account
            };
    ...
    peernet.get_transport = function (contact_transport, contact_address, contact_port) {
        ...
        if (!contact_transport) {
            ...
            throw new Error("invalid contact protocol, the contact protocol cannot be null");
        }
        ...
        //  the contact uses TCP client, this users uses TCP client the 
        //  PeerNet instance must exist and return that
        if (!(peernet.netclient instanceof PeerNet)) {
            throw new Error("invalid PeerNet instance");
        }
    ...
    peernet.init = function () {
        ...
        return new Promise((resolve, reject) => {
            logger.debug("Initialize Mumble Network");
            ...
            peernet.connect(settings.transport, function (err) {
                ...
                // it was an error
                if (peernet.netclient instanceof WsNet ||
                    (peernet.netclient instanceof PeerNet && settings.transport == defs.TRANSPORT_TCP && (!settings.wsfallback && settings.ask_about_wsfallback))) {
                    // it seems the Websocket connection failed or the TCP failed and the WS connection is not allowed
                    return reject("Error in connecting to the Mumble network, " + (err.message ? err.message : err));
                }
                else if ((peernet.netclient instanceof PeerNet) && settings.transport == defs.TRANSPORT_TCP && settings.wsfallback && settings.ask_about_wsfallback) {
                    ...
                    if (!usews) {
                        return reject("Not connected to the Mumble network. Error: " + (err.message ? err.message : err));
                    }
    ...
    peernet.ping_to_net = function () {
        if (!appsrvc.net_connected) {
            return alert("Unable to ping. You are not connected to the Mumble network");
        }
        alert("ping to net");
    };
