...
Node.prototype._putValidatedKeyValue = function (item, callback) {
    ...
    var cbcompletecount = 0;
    var completed = {};

    var putmsg = function (contact, donecb) {
        if (completed[contact.nodeID]) {
            return donecb(null, { error: 0, success: true });
        }
        ...
        node._rpc.send(contact, message, function (err) {
            try {
                if (err) {
                    ...
                    return donecb(null, { error: err.message || err, success: false });
                }

                // one more completed
                completed[contact.nodeID] = Date.now();
                cbcompletecount++;
                node._log.debug('STORE complete for ' + contact.address + ' ' + contact.nodeID);

                //  if there is no error and at least two operation is completed then 
                //  return this STORE operation to the UI and continue with the remaining nodes
                if (!cbreturned && cbcompletecount >= 2) {
                    cbreturned = true;
                    callback();
                    node._log.debug('two STORE succeeded -> return the operation immediately');
                }
                ...
                donecb(null, { error: e.message, success: false });
            }
        });
    };
