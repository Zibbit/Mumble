...
(function () {
    ...
    apputils.clear_database = function () {
        var process = confirm("All settings and data will be removed from the local database. We strongly suggest to clear the database if you use Mumble on a shared/public computer.  Make sure you made a backup if you want to keep using your account.");
        if (process) {
            database.clear().then(
                function () {
                    mumble.notify.success("The database was cleared successfully");
                },
                function (err) {
                    mumble.notify.error("Database clear error %j", err);
                }
            );
        }
    };
    ...
    apputils.backup_account = function() {
        try {
            ...
            filedialog.initialize({
                ...
                accept: ['mumble.dat'],
                path: '~/Documents',
                defaultSavePath: 'mumble.dat'
            });

            var objext = JSON.stringify(account);
            var encoded = window.btoa(objext);

            var text = "---BEGIN MUMBLE KEY---\n";
            text += encoded;
            text += "\n---END MUMBLE KEY---";

            var file_name = "mumble_" + username + ".dat";
            filedialog.saveTextFile(text, file_name, function () {
                mumble.notify.success("Account backup is completed.");
            });            

        }
        catch (err) {
            mumble.notify.error("Account backup error: %j", err);
        }       
    }
    ...
    apputils.backup_contacts = function () {
        try {
            ...
            database.ContactsDB.get_contacts(username, function (err, result) {
                if (err) {
                    return mumble.notify.error("Database get contacts error %j", err, true);
                }
                ...
                filedialog.saveTextFile(text, file_name, function () {
                    mumble.notify.success("Contacts backup is completed.", null, true);
                });
            });            
    ...
    apputils.get_pending_contacts = function (callback) {
        database.getall(database.TEMPCONTDB, function (err, result) {
            if (err) {
                ...
                return mumble.notify.error("Database get pending contacts error %j", err, true);
            }
    ...
    apputils.get_pending_contact = function (account) {
        ...
        try {
            ...
            mumble.notify.error("Get pending contact error: %j", e, true);
        }
    }
    ...
    apputils.add_blocked_contact = function (contact_name, callback) {
        ...
        database.update(database.BLOCKCONTDB, blocked_contacts).then(
            ...
            function (err) {
                mumble.notify.error("Update blocked contacts error %j", err, true);
                callback(err);
            }
        );
    }
    ...
    apputils.accept_call = function (sender, type, resultfn) {
        ...
        if (type != defs.CALLTYPE_VIDEO && type != defs.CALLTYPE_AUDIO) {
            return mumble.notify.error("Invalid call type received from " + sender, null, true);
        }
    ...
    apputils.restore_account = function (callback) {
        try {
            ...
            filedialog.readTextFile(function (err, buffer, path) {
                ...
                try {
                    if (!buffer) {
                        throw new Error("not a valid Mumble account backup");
                    }

                    var data = (typeof buffer == "string") ? buffer : buffer.toString();
                    var find1 = "---BEGIN MUMBLE KEY---\n";
                    ...
                    return mumble.notify.error("Invalid account backup data. Error: %j", e);
                }

                if (!text) {
                    return mumble.notify.error("Invalid account backup data");
                }
            ...
            mumble.notify.error("Account restore error: %j", e);
        }
    }
    ...
    apputils.restore_contacts = function (callback) {
        try {
            ...
            filedialog.readTextFile(function (err, buffer, path) {
                ...
                try {
                    if (!buffer) {
                        throw new Error("not a valid Mumble contact backup");
                    }
                    ...
                    return mumble.notify.error("Mumble contacts restore error: %j", e);
                }
                ...
                bootbox.confirm("Restore the contacts list. All existing contact data will be overwritten", function (result) {
                    if (result) {
                        ...
                        database.ContactsDB.update(username, data, function (err) {
                            if (err) {
                                return mumble.notify.error("Contacts restore error: %j", e);
                            }

                            mumble.notify.success("The contacts have been restored.");
                        });
                    }
                });
            });              
            ...
            mumble.notify.error("Contacts restore error: %j", e);
        }
    }
    ...
    apputils.getversion = function (callback) {
        ...
        $.getJSON('https://api.github.com/repos/mumble/mumbleui/releases', {}, function (data) {
            var version;
