...
(function () {
    ...
    function addToDB(account, publickey, cipher_context, callback) {
        ...
        accounts.update(user, function (err) {
            if (err) {
                return mumble.notify.error("Database update error %j", err);
            }
    ...
    usrobj.create_account = function (account, password, callback) {
        try {
            ...
            addToDB(account, publicKey, cipher_context, function () {
                ...
                mumble.ui.set_account_title(account);
    ...
    usrobj.initialize = function (user, password) {
        try {
            if (!user || !password) {
                mumble.notify.error("Invalid parameters, the account and passwords are required");
                return false;
            }

            var account_name = user.account;
            if (!account_name) {
                mumble.notify.error("Invalid account name");
                return false;
            }
            ...
            try {
                ...
                if (err.message && err.message.indexOf("decrypt") > -1) {
                    mumble.notify.error("Account initialize error, most likely an incorrect password was entered");
                }
                else {
                    mumble.notify.error("Account initialize error: %j", err);
                }
                return false;
            }
            ...
            try {
                ...
                mumble.notify.error("Account initialize error. Select a saved account and enter the valid password. The encrypted account information must exists on the computer.");
                return false;
            }
            ...
            if (key.publicKeyHex != user.public_key) {
                mumble.notify.error("Error in initializing the account, most likely an incorrect password");
                return false;
            }
            ...
            mumble.ui.set_account_title(account_name);

            // the account exists and the encrypted entropy is correct!
            return true;
        }
        catch (err) {
            mumble.notify.error("Account initialize error. Select a saved account and enter the valid password.");
            return false;
        }
    };
    ...
    usrobj.restore = function (password, user, callback) {
        try {
            ...
            key.keyFromPrivate(hexPrivatekey, 'hex');
            if (key.publicKeyHex != user.public_key) {
                mumble.notify.error("Error in restoring the account, incorrect password or invalid backup data");
                return false;
            }
            
            if (!userobj.connsymmkey) {
                mumble.notify.error("Error in restoring the account, incorrect connection key in backup data");
                return false;
            }
            ...
            addToDB(account, publicKey, cipher_context, function () {
                ...
                mumble.ui.set_account_title(account);
                mumble.notify.success("The account has been restored and initialized. You can connect to the Mumble network");
    ...
    usrobj.change_password = function (user, password, callback) {
        try {
            ...
            if (usrobj.name != account) {
                mumble.notify.error("Account " + account + " is not initialized");
                return false;
            }
            ...
            if (!appsrvc.privateKeyHex || !appsrvc.connsymmkey) {
                mumble.notify.error("the existing crypto key is invalid");
                return false;
            }
