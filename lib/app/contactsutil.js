...
(function () {
    ...
    function validate_contact(account, obj) {
        if (!account || !obj) {
            return mumble.notify.error("validate_contact error: invalid parameters");
        }
    ...
    function init_contact(contact, callback) {
        try {
            ...
            contactobj.find(function(err, result){
                if(err) {
                    var msg = mumble.notify.error("Couldn't find contact " + account + ", %j", err, true);
                ...
                if (!result || account != result.name) {
                    ...
                    mumble.notify.error("Couldn't find contact " + account + " on the network", null, true);
                    return callback();
                }
            ...
            callback();
            mumble.notify.error("init_contact error: %j", err, true);
        }
    }
    ...
    handler.add_contact = function (contact, cbfunc) {
        ...
        function (err) {
            if (err && !isoffer_complete) {
                ...
                mumble.notify.info("Contact offer was added, but offer from the contact was not received", null, true);
            }
    ...
    handler.init = function (callback) {
        try {
            ...
            async.eachSeries(contacts, init_contact, function (err) {
                ...
                mumble.notify.info(msg, null, true);

                //
            });
        }
        catch (err) {
            mumble.notify.error("Error in initializing contacts: %j", err, true);
        }
    }
