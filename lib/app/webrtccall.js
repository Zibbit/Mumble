...
(function () {
                ...
                connection.createOffer(offerOptions).then(
                    function (desc) {
                        try {
                            ...
                            mumble.notify.error("setLocalDescription error: %j", err, true);
                        }
                    },
                    function (error) {
    ...
    function onCandidateSignalReceived(connection, candidate) {
        ...
        connection.addIceCandidate(new RTCIceCandidate(candidate)).then(
            ...
            function (err) {
                mumble.notify.error("WEBRTC: addIceCandidate error: %j", err, true);
            }
        );
    }
    ...
    function onSdpSignalReceived(connection, sdp) {
        ...
        try {
            ...
            mumble.notify.error("setRemoteDescription error: %j", err, true);
        }
    }
    ...
    webrtccall.show_video = function (callback) {
        try {
            ...
            mumble.notify.error("Show video error: %j", err, true);
        }
    }
    ...
    webrtccall.hide_video = function (callback) {
        try {
            ...
            mumble.notify.error("Hide video error: %j", err, true);
        }
    }
    ...
    function onReadyForStream(connection) {
        try {
            if (!mediaStream) {
                return mumble.notify.error_popup('Invalid media stream');
            }
    ...
    function call_contact() {
        try {
            if (!mediaStream) {
                return mumble.notify.error('Invalid media stream');
            }
            ...
            mumble.notify.error("call_contact error %j", err);
        }
    }
    ...
    webrtccall.initcall = function (localvideo, contactvideo, options) {
        try {
            ...
            navigator.mediaDevices.getUserMedia(params)
                ...
                .catch(function (error) {
                    if (error.name === 'ConstraintNotSatisfiedError') {
                        ...
                        mumble.notify.error('Get user media error: %j', error);
                    }
                });
            ...
            mumble.notify.error("Media call initialization error: %j", err);
        }
    }
